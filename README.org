...
2. Follow the instructions in the =INSTALL= file to build emacs.
   - There may be some issues with permissions and dependencies so pay
     attention.
...
- =emacs-28.0.60= requested =libacl1-dev= and =libharfbuzz-dev= be installed but
  worked perfectly after that. It does feel snappier. It told me that my current
  version of GTK+ leads to a bug but I couldn't figure out how to update GTK+
  and it seemed to be an up to date version anyway. This hasn't caused problems
  though... yet.
...
#  LocalWords:  nicemacs Spacemacs spacemacs SPC
