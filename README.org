...
=ghcup= is a tool to manage haskell versions and the haskell language server.
The bin directory for this tool needs to be added to the path so emacs can find
the haskell language server program.
...
style, so you probably just want to do that manually.
