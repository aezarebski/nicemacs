...
Apparently, this is [[https://xkcd.com/1205/][worth the time...]]

#+begin_src emacs-lisp :tangle nicemacs.el
(defun journal-commit-message ()
  "Insert a string describing a generic commit to my journal
file."
  (interactive)
  (let ((date-string (downcase (format-time-string "%A %l:%M %p"))))
    (insert (format "update tasklist %s"
                    date-string))))

(spacemacs/set-leader-keys "oocm" 'journal-commit-message)
#+end_src
