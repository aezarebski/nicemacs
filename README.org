...
The following settings assume that you have =maxima=, =maxima-doc= and
=maxima-emacs= installed via your package manager. These settings are probably
quite specific to linux, but the snippet below might be useful if you want to
use MacOS. Basically, the =maxima= command starts a maxima REPL and the file
extensions =.max= and =.mac= are taken as refering to maxima files.
...
This seems to be where brew installed maxima on a mac I was using.
...
*** [[file:./maxima/README.org][Maxima layer]]
