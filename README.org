...
- The =hackernews= layer provides an application for reading hacker news
  + the application is bound to =SPC a w h=
  + the variable =hackernews-items-per-page= describes how many to display.
...
And some for specifically visiting the website and associated notes files
...
For example, to insert a lower case alpha, you would use =(insert-greek "small"
"alpha")=. Then we can add key bindings for this =SPC o u <l>= to insert the
Greek letter.
