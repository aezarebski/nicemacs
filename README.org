...
This function opens a buffer for the current month and attempts to center the
cursor at the entry for the current date."
  (interactive)
  (let ((path-template "~/Documents/journal/journal-%s.org")
        (year-month (format-time-string "%Y-%m"))
        (entry-regex (format-time-string "^** %d")))
    (find-file (format path-template year-month))
    (goto-char 1)
    (re-search-forward entry-regex nil t)
    (recenter-top-bottom)))
...
;; Create a table.
(spacemacs/set-leader-keys "ooit" 'org-table-create-or-convert-from-region)
...
(defun org-insert-source-block ()
  ...
  (progn
    ...
    (evil-forward-char 10)
    (insert " ")))

(spacemacs/declare-prefix "ooi" "orgo-inserto-menu")
(spacemacs/set-leader-keys "oois" 'org-insert-source-block)
