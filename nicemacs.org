#+title: NICEMACS
#+Time-stamp: <Last modified: 2022-05-17 12:55:42>
#+startup: overview
#+OPTIONS: toc:nil
#+OPTIONS: num:nil
...
used to specify which files to ignore using a regular expression. The =htmlize=
package seems to need to be manually imported to get syntax highlighting to
work.
...
To visit a file with sudo rights use =SPC f E= which calls =spacemacs/sudo-edit=.
...
There is a package, =treemacs=, which provides a graphical way to visit frequently
used directories, but after using it for a little while I did not find it that
comfortable. The following method combined with dired seems much nicer. The
following functionality is useful for defining visitors of frequently needed
files.
...
- =SPC t l= will toggle long lines.
...
Sadly this task gets mangled on HTML output, but hopefully is still readable
from source.

#+begin_src snippet :tangle ~/.emacs.d/private/snippets/org-mode/nicemacs-task
# -*- mode: snippet -*-
# name: Include a task for the journal
# key: nicemacs-task
# --
** TODO [#B] do this thing
#+end_src
...
#  LocalWords:  defmacro backend LSP Bibtex
