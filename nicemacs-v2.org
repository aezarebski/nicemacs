...
- If you want to bulk unstage files in the magit status buffer, you
  can use =C-SPC= (to call the =set-mark-command=), select a bunch of
  consecutive files, and then unstage them with =u=.
