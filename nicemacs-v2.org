...
Meld provides a convenient GUI based method for assessing the
differences between files. Although, I have switched to using Kompare
because when I shifted machine Meld started bugging out and Kompare
seems simpler (although has a clunkier UI than Meld).
...
In the following configuration of =magit=, notice how the
=SSH_AUTH_SOCK= variable is set. The correct value for this comes from
asking what the corresponding value is in a regular terminal with
=echo=.
...
During REPL stuff, activate the virtual environment with =m v a=; you
can start a REPL with =C-c C-p=, which runs =run-python= (from within
a python-mode buffer); to kill the REPL use =C-c C-\= (from within an
inferior process, which runs =comint-quit-subjob=.
...
- The functions =org-footnote-new= creates a new footnote (obviously),
  less obvious are the very helpful =org-footnote-normalize= and
  =org-footnote-renumber= which will help you sort out your shambolic
  footnotes.
...
- You will need to have the Anki application running to use this
  package fully.
- =anki-editor-sync-collection= synchronizes the local version to the
  web collection (so seems to be more of a "pull" operation).
- =anki-editor-push-notes= is needed to push the current collection to
  the web.
...
=bookmarks.el= provides functions to create bookmarks in buffers and
then navigate between them in subsequent sessions. The core functions
you need to know are

- =bookmark-set= to create a (named) bookmark
- =bookmark-jump= to jump to a bookmark
- =bookmark-delete= to delete a bookmark
- =bookmark-bmenu-list= to list bookmarks

* ChangeLog

** 2025-07

- Add use of =csv-mode= which provides column highlighting.

** 2025-06

- Improve the way that theme and fonts can be toggled with
  =nice-toggle-theme= and =nice-toggle-font=.
...
- Add the =nice-connect-brahms= function as a way to make it easier to
  connect to servers via TRAMP.
- Ensure that =copilot=, =htmlize= and =ligature= are all installed
  and configured properly.

** 2024-07

- Use =openwith= so that =dired= will open PDFs with Okular on click
  rather than the emacs PDF viewer.

** 2024-05

- Add binding of =SPC m s s= to evaluate R source blocks in org-mode.
  (This is =C-c C-c= by default.)
- Add some ESS keybindings to =markdown-mode= so they are available
  when writing Rmarkdown.
- Bug fix where ESS couldn't find the directory where the ESS-R files
  are. This is now fixed by looking up a plausible directory and
  setting =ess-etc-directory= to that.

** 2024-03

- Use =org-agenda-start-with-log-mode= to show the log mode items in
  the agenda.
- =.Rmd= files should also open in =markdown-mode=.
- Use the =:exclude= pattern to be a bit more selective with the files
  that get copied by =org-publish= (i.e. do not accidentally copy a
  whole Python virtual environment.)
...
- Include emoji keybinding amount yasnippet bindings since they play a
  similar role.
...
- Add a scrartcl class to =org-latex-classes= as nicer org-mode PDF
  export option.

** 2023-09

- Use =evil-window-X= functions as a simpler alternative to =winum=.
...
- Avoid annoying warning from =git= by setting =GIT_PAGER= to =cat=.
...
- Extend =before-save-hook= to avoid accidental trailing whitespace.
